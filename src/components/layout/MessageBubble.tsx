...
export function MessageBubble({ content, isUser, avatarSrc, isTyping, lastMessage }: MessageBubbleProps) {
  ...
  const customComponents = (lastMessage: boolean) => ({
    ...
    p: ({ children, ...props }: CustomComponentProps<HTMLParagraphElement>) => {
      const textContent = React.Children.toArray(children).join(""); // Ensure it's a clean string
      const safeTextContent = textContent || ""; // Ensure it's a string
      return lastMessage ? (
        <TypingText text={safeTextContent} />
      ) : (
    ...
    >
      {isTyping ? (
        ...
        <ReactMarkdown
          ...
          components={customComponents(!!lastMessage)}
        >
          {formattedContent}
        </ReactMarkdown>
      )}
